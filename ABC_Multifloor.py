...
class MapWindow(QMainWindow):
    def __init__(self):
        ...
        self.ROOM_ID_COL = "ABC2ROOMS"  # Column name for Room IDs in the rooms shapefile
        ...
        try:
            ...
            self.nodes = gpd.read_file("ABCMERGEDNODES.shp")
            
            # Load the single merged edge file 
            self.edges = gpd.read_file("ABCMERGEDEDGES.shp")
            
            # Load the single merged room file
            self.rooms = gpd.read_file("ABCMERGEDROOMS.shp")
            
            # *** NEW: Load the single merged restricted area file ***
            self.restricted_areas = gpd.read_file("ABCMERGEDRESTRICTEDAREA.shp")
