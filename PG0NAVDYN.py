...
class MapWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("🏢 Dynamic Indoor Navigation GUI (PG GROUND Floor)")

        # Define the expected names for ID columns
        self.NODE_ID_COL = "PG0NODES"  
        self.EDGE_ID_COL = "PG0EDGES"
        self.ROOM_ID_COL = "PG0ROOMS"  # **ASSUMED COLUMN NAME for room labels**

        # ---------------- Load Data ----------------
        try:
            # 1. Core data for graph
            self.nodes = gpd.read_file("PG_GROUND_NODES.shp")
            self.edges = gpd.read_file("PG_GROUND_EDGES.shp")
            
            # 2. Visualization data
            self.rooms = gpd.read_file("PG_GROUND_ROOMS.shp")
            self.restricted_areas = gpd.read_file("PG_GROUND_RESTRICTEDAREA.shp")
    ...
    def plot_map(self, path=None):
        ...
        self.ax.set_title("Navigation Map (PG GROUND Floor)")
