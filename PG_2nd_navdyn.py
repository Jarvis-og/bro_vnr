...
class MapWindow(QMainWindow):
    def __init__(self):
        ...
        self.NODE_ID_COL = "Node ID"  
        self.EDGE_ID_COL = "Edge ID" 
        self.ROOM_ID_COL = "Room"  # Column name for Room IDs in the rooms shapefile

        # ---------------- Load Data ----------------
        try:
            # Load basic node and edge data
            self.nodes = gpd.read_file("PG_2nd_Nodes.shp")
            self.edges = gpd.read_file("PG_2nd_Edges.shp")
            
            # Load visualization data (Rooms and Restricted Areas)
            self.rooms = gpd.read_file("PG_2nd_Rooms.shp")
            self.restricted_areas = gpd.read_file("PG_2nd_Restricted.shp")
    ...
    def plot_map(self, path=None):
        ...
        # Add title
        self.ax.set_title("PG2nd Floor Navigation Map")
